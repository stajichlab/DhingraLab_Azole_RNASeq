...
calcPlotPairwise <- function(c) {
  
  sampleTablePair = sampleTable %>% filter(condition == c)
  ...
  res <- results(dds)
  res    
  res <- results(dds, contrast=c("genotype","AF293","CEA10"))
  res
  
  resultsNames(dds)
  resLFC <- lfcShrink(dds, coef="genotype_CEA10_vs_AF293", type="apeglm")
  resLFC
  summary(resLFC)
  summary(res)
  ...
  plotMA(res, ylim=c(-2,2),main=sprintf("%s CEA10 vs AF293",c))
  plotMA(resLFC, ylim=c(-2,2),main=sprintf("%s CEA10 vs AF293",c))
  
  resSig <- subset(resLFC, padj < 0.05)
  resSig <- resSig[order(resSig$pvalue),]
  write.csv(resSig,sprintf("reports/%s_CEA10_vs_AF293.csv",c))
  write.csv(fpm(dds),sprintf("reports/%s_allgeno_FPM.csv",c))
  dev.off()
}
lapply(unique(treatment),calcPlotPairwise)
